...
// Use Mocha's before instead of beforeAll
before(async () => {
    // Ensure you start the server and get the address and port dynamically
    const { address, port } = await server.address();
    baseUrl = `http://${address === '::' ? 'localhost' : address}:${port}`;
...
describe('Resource API', () => {
    describe('PUT /notes/:id', () => {
        it('should update a note successfully', async () => {
            const response = await request(app).put('/notes/1731063467903805').send({
                title: 'Updated Title',
                description: 'Updated Description',
                priority: 'high'
            });
        ...
        it('should return 404 if note does not exist', async () => {
            const response = await request(app).put('/notes/9999999999999999').send({
                title: 'Non-existing Title',
                description: 'Non-existing Description',
                priority: 'low'
            });
